const BUILD_VERSION=51;window.VILNA_DAF_BUILD=BUILD_VERSION;
...
$("buildBadge").textContent=`Build ${BUILD_VERSION}`;document.title=`Bava Metzia 21a Studio - Build ${BUILD_VERSION}`;
...
function setScale(scale){const p=$("dafPage"),s=scaleParts(scale),type=referenceProfile()?.typography||{},gemaraSize=type.gemaraSize||15.1,commentarySize=type.commentarySize||11.05,gemaraLeading=type.gemaraLeading||16.35,commentaryLeading=type.commentaryLeading||12.05;p.style.setProperty("--gemara-size",`${gemaraSize*s.gemara}px`);p.style.setProperty("--gemara-leading",`${gemaraLeading*s.gemara}px`);p.style.setProperty("--commentary-size",`${commentarySize*s.commentary}px`);p.style.setProperty("--commentary-leading",`${commentaryLeading*s.commentary}px`);}
...
async function loadDaf({automatic=false}={}){const ref=$("dafRef").value.trim();if(!/\d+[ab]\s*$/i.test(ref)){status("Use a Talmud reference ending in a or b, such as Pesachim 99b.",true);return;}$("loadDaf").disabled=true;status("Step 1 of 2 — loading complete Gemara, Rashi, Tosafos, and available Rashbam from Sefaria…");try{const tractate=ref.replace(/\s+\d+[ab]\s*$/i,""),location=ref.match(/\d+[ab]\s*$/i)[0].trim(),commentary={commentary:true},requests=[fetchText(ref),fetchText(`Rashi on ${tractate} ${location}`,commentary),fetchText(`Tosafot on ${tractate} ${location}`,commentary),fetchText(`Rashbam on ${tractate} ${location}`,commentary).catch(()=>({html:""}))],[g,r,t,b]=await Promise.all(requests);if(!g.html)throw new Error("No Hebrew Gemara text was returned");const gemaraHtml=normalizeGemaraForRef(g.html,ref),rashiHtml=normalizeCommentaryForRef(r.html,ref,"inner");Object.assign(state,{ref,header:headerFor(ref,g),gemaraHtml,rashiHtml,tosafotHtml:t.html,rashbamHtml:b.html,isSample:false,selectedWordId:null,editSelectedWordIds:[],wordFontScales:{},whitedWordIds:{},notes:{},annotations:[],annotationUndo:[],annotationRedo:[],selection:null});syncRegistrySelection(ref);clearExcerpt();syncAnnotationCanvas();status(`Step 2 of 2 — text loaded${b.html?" with Rashbam":""}; starting the composition test…`);await nextPaint();await compose();}catch(e){setComposing(false);status(`${automatic?"Automatic import failed":"Could not load this daf"}: ${e.message}. The demonstration text remains available.`,true);}finally{$("loadDaf").disabled=false;}}
...
async function openProjectFile(file){const data=JSON.parse(await file.text());if(data?.format!=="vilna-daf-studio-project"||data.version!==1||!data.daf?.ref)throw new Error("This is not a supported Vilna Daf Studio project");const projectRef=String(data.daf.ref);Object.assign(state,{ref:projectRef,header:String(data.daf.header||""),gemaraHtml:normalizeGemaraForRef(cleanHtml(data.daf.gemaraHtml),projectRef),rashiHtml:normalizeCommentaryForRef(cleanHtml(data.daf.rashiHtml),projectRef,"inner"),rashbamHtml:cleanHtml(data.daf.rashbamHtml),tosafotHtml:cleanHtml(data.daf.tosafotHtml),isSample:Boolean(data.daf.isSample),typography:TYPOGRAPHY_PRESETS[data.view?.typography]?data.view.typography:"archival-open",mode:data.view?.mode==="edit"?"edit":"navigate",navigationUnit:data.view?.navigationUnit==="phrase"?"phrase":"word",selectedWordId:data.view?.selectedWordId||null,editSelectedWordIds:[],wordFontScales:data.view?.wordFontScales&&typeof data.view.wordFontScales==="object"?data.view.wordFontScales:{},whitedWordIds:data.view?.whitedWordIds&&typeof data.view.whitedWordIds==="object"?data.view.whitedWordIds:{},notesEnabled:Boolean(data.view?.notesEnabled),nekudosEnabled:data.view?.nekudosEnabled!==false,lineNumbersEnabled:Boolean(data.view?.lineNumbersEnabled),pageZoom:normalizedPageZoom(data.view?.pageZoom),noteWindowPosition:data.view?.noteWindowPosition||null,selection:data.view?.selection||null,notes:data.notes&&typeof data.notes==="object"?data.notes:{},annotations:Array.isArray(data.annotations)?data.annotations:[],annotationTool:["pen","highlighter","eraser"].includes(data.annotationSettings?.tool)?data.annotationSettings.tool:"pen",annotationColor:data.annotationSettings?.color||"#b32424",strokeWidth:Number(data.annotationSettings?.width)||3,annotationUndo:[],annotationRedo:[]});$("dafRef").value=state.ref;syncRegistrySelection(state.ref);$("annotationColor").value=state.annotationColor;$("strokeWidth").value=state.strokeWidth;$("strokeWidthValue").textContent=state.strokeWidth;updateDisplayToggles();selectAnnotationTool(state.annotationTool);setAnnotating(false);await applyTypography(state.typography,{recompose:false});await compose();toggleNotes(state.notesEnabled);if(state.selection){const b=$("selectionBox");b.hidden=false;Object.assign(b.style,{left:`${state.selection.x}px`,top:`${state.selection.y}px`,width:`${state.selection.width}px`,height:`${state.selection.height}px`});["copyExcerpt","downloadExcerpt","clearExcerpt"].forEach(id=>$(id).disabled=false);}$("projectStatus").textContent=`Opened ${file.name}.`;}
