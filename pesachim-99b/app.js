const BUILD_VERSION=44;window.VILNA_DAF_BUILD=BUILD_VERSION;
...
const state={...sample,selection:null,selecting:false,composition:null,typography:TYPOGRAPHY_PRESETS[savedTypography]?savedTypography:"archival-open",mode:"navigate",navigationUnit:"word",selectedWordId:null,editSelectedWordIds:[],wordFontScales:{},whitedWordIds:{},notesEnabled:false,nekudosEnabled:true,lineNumbersEnabled:false,pageZoom:1.5,notes:{},noteWindowPosition:null,annotating:false,annotationTool:"pen",annotationColor:"#b32424",strokeWidth:3,annotations:[],annotationUndo:[],annotationRedo:[]};
...
$("buildBadge").textContent=`Build ${BUILD_VERSION}`;document.title=`Vilna Daf Studio - Build ${BUILD_VERSION}`;
...
function auditReferenceProfile(profile){
  ...
  for(const stream of STREAMS){const map=maps[stream],ends=map?.lineEndTokens||[];if(!map||ends.at(-1)!==map.tokenCount-1||ends.some((end,i)=>!Number.isInteger(end)||end<0||(i&&end<=ends[i-1])))errors.push(`${stream} token coverage`);}
  const innerSlots=maps.inner.lineEndTokens.length+(maps.inner.blankAfterTokens?.length||0),tosafotSlots=maps.tosafot.lineEndTokens.length+(profile.notice?.lines||0);
  if(maps.gemara.lineEndTokens.length!==23)errors.push("Gemara PDF line count");
  if(innerSlots>layout.openingLines+layout.narrowLines+layout.pairLines||layout.openingLines+layout.narrowLines+layout.pairLines-innerSlots!==1)errors.push("Rashi/Rashbam stage allocation");
  if(tosafotSlots!==layout.openingLines+layout.narrowLines+layout.pairLines+layout.fullLines)errors.push("Tosafos stage allocation");
  const lengths=map=>map.lineEndTokens.map((end,i)=>end-(i?map.lineEndTokens[i-1]:-1));
  if(Math.max(...lengths(maps.inner))>14)errors.push("Rashi/Rashbam implausible line length");
  const tosLengths=lengths(maps.tosafot),fullStart=layout.openingLines+layout.narrowLines+layout.pairLines-(profile.notice?.lines||0);if(Math.max(...tosLengths.slice(0,fullStart))>12||Math.max(...tosLengths.slice(fullStart))>25)errors.push("Tosafos implausible line length");
  return errors;
}
...
function mappedLinesHtml(lines){return lines.map((lineTokens,index)=>`<span class="mapped-line mapped-line-${index+1}${lineTokens===null?" mapped-blank-line":""}${lineTokens?.commentEnd?" mapped-comment-end-line":""}">${lineTokens===null?"":tokensHtml(lineTokens)}</span>`).join("");}
...
function fitMappedLineWidths(region){if(!region?.classList.contains("reference-mapped"))return;for(const line of region.querySelectorAll(":scope > .mapped-line")){line.style.transform="";line.dataset.widthScale="1";line.dataset.requiredScale="1";const available=line.clientWidth,natural=line.scrollWidth;if(available>0&&natural>available+.5){const required=available/natural,scale=Math.max(.88,required);line.style.transformOrigin="right center";line.style.transform=`scaleX(${scale})`;line.dataset.widthScale=String(scale);line.dataset.requiredScale=String(required);}}}
...
function setScale(scale){const p=$("dafPage"),s=scaleParts(scale),type=referenceProfile()?.typography||{},gemaraLeading=type.gemaraLeading||16.35,commentaryLeading=type.commentaryLeading||12.05;p.style.setProperty("--gemara-size",`${15.1*s.gemara}px`);p.style.setProperty("--gemara-leading",`${gemaraLeading*s.gemara}px`);p.style.setProperty("--commentary-size",`${11.05*s.commentary}px`);p.style.setProperty("--commentary-leading",`${commentaryLeading*s.commentary}px`);}
...
function fitAllRemainingText(region,html){if(!region||!html)return true;if(referenceProfile())return false;region.innerHTML=`${region.innerHTML} ${html}`.trim();const cs=getComputedStyle(region),baseSize=parseFloat(cs.fontSize)||11,baseLine=parseFloat(cs.lineHeight)||12;for(let factor=1;factor>=.84;factor-=.02){region.style.fontSize=`${baseSize*factor}px`;region.style.lineHeight=`${baseLine*factor}px`;if(region.scrollHeight<=region.clientHeight+1){finishRegionLine(region,false);return true;}}return false;}
function fillStream(tokens,regions,shapeLines=false){let rest=tokens,blankArea=0,totalArea=0;const occupancy=[],filled=[];for(const region of regions){const r=fitTokens(rest,region);region.innerHTML=renderedTokens(r.chunk,region);fitMappedLineWidths(region);rest=r.rest;filled.push({region,continues:rest.length>0});const ratio=region.clientHeight?Math.min(1,r.usedHeight/region.clientHeight):0,area=region.clientWidth*region.clientHeight;occupancy.push(ratio);totalArea+=area;blankArea+=area*(1-ratio);}if(shapeLines)filled.forEach(({region,continues})=>finishRegionLine(region,continues));return{rest,blankArea,totalArea,occupancy};}

function withoutBreaks(html){return html.replace(/<br\s*\/?\s*>/gi," ").replace(/\s+/g," ").trim();}
function innerHtml(){const r=withoutBreaks(state.rashiHtml.trim()),b=withoutBreaks(state.rashbamHtml.trim());if(!b)return r;if(!r)return b;return`${r}<span class="rashbam-transition"><br><br><span class="commentary-heading"><strong>פירוש רבינו שמואל<br>תלמיד רש״י ז״ל</strong></span><br></span>${b}`;}
function streamTokens(){const streams={gemara:tokenize(displayHtml(withoutBreaks(state.gemaraHtml))),inner:tokenize(displayHtml(innerHtml())),tosafot:tokenize(displayHtml(withoutBreaks(state.tosafotHtml)))},profile=referenceProfile();if(profile)for(const stream of STREAMS)streams[stream]=applyExactLineMap(streams[stream],profile.maps?.[stream],stream);return streams;}
...
function mappedCascadePattern(profile){
  ...
  const layout=profile.layout,gemaraLines=profile.maps.gemara.lineEndTokens.length,outerHeight=gutter+gemaraLines*gemaraLeading+gutter,narrowHeight=layout.narrowLines*commentaryLeading;
  if(Math.abs(outerHeight-narrowHeight)>1)throw new Error(`Mapped box mismatch: Gemara requires ${outerHeight.toFixed(1)}px but commentary requires ${narrowHeight.toFixed(1)}px`);
  return{name:`PDF-mapped cascading ${layout.survivor} takeover`,cascade:true,mapped:true,bands:[
    {height:0,pixelHeight:outerHeight,streams:ordered(STREAMS),widths:primaryWidths()},
    {height:0,pixelHeight:layout.pairLines*commentaryLeading,streams:ordered(["inner","tosafot"]),widths:[50,50]},
    {height:0,streams:[layout.survivor],widths:[100]}
  ]};
}
function candidates(w){const ranked=[...STREAMS].sort((a,b)=>w[b]-w[a]),three=ordered(STREAMS),top=primaryWidths(w),out=[{name:"continuous three-column",bands:[{height:100,streams:three,widths:top}]}];for(const cut of stepped(32,86,6))for(const owner of STREAMS)out.push({name:`${owner} lower continuation`,bands:[{height:cut,streams:three,widths:top},{height:100-cut,streams:[owner],widths:[100]}]});for(const cut of stepped(28,82,6))for(const omitted of STREAMS){const lower=ordered(STREAMS.filter(s=>s!==omitted));out.push({name:`${lower.join(" + ")} lower band`,bands:[{height:cut,streams:three,widths:top},{height:100-cut,streams:lower,widths:pairWidths(lower,w)}]});}const pair=ordered(ranked.slice(0,2));for(const a of stepped(30,62,8))for(const b of stepped(16,40,8))if(a+b<92)out.push({name:`${ranked[0]} extended three-stage`,bands:[{height:a,streams:three,widths:top},{height:b,streams:pair,widths:pairWidths(pair,w)},{height:100-a-b,streams:[ranked[0]],widths:[100]}]});const commentaryPair=ordered(["inner","tosafot"]);for(const upper of stepped(28,60,8))for(const middle of stepped(12,44,8))if(upper+middle<94)for(const survivor of["inner","tosafot"])out.push({name:`cascading ${survivor} takeover`,cascade:true,bands:[{height:upper,streams:three,widths:top},{height:middle,streams:commentaryPair,widths:[50,50]},{height:100-upper-middle,streams:[survivor],widths:[100]}]});return out;}
function buildGeometry(pattern){
  ...
  body.innerHTML="";body.classList.toggle("mapped-geometry",Boolean(pattern.mapped));
...
function composeMappedExact(tokens,profile){
  ...
  const {openingLines,narrowLines,pairLines,fullLines,survivor}=profile.layout;
  const noticeLines=profile.notice?.lines||0,expected={gemara:profile.maps.gemara.lineEndTokens.length,inner:openingLines+narrowLines+pairLines-1,tosafot:openingLines+narrowLines+pairLines+fullLines-noticeLines};
  for(const stream of STREAMS)if(lines[stream].length!==expected[stream])throw new Error(`Mapped line allocation mismatch for ${stream}: expected ${expected[stream]}, received ${lines[stream].length}`);
  const right=isAleph(state.ref)?"inner":"tosafot",left=isAleph(state.ref)?"tosafot":"inner";
  for(const [element,stream] of [[$("topRight"),right],[$("topLeft"),left]]){element.dataset.stream=stream;element.classList.add("reference-mapped");const offset=stream===profile.notice?.stream?noticeLines:0,opening=lines[stream].slice(0,openingLines-offset);setMappedLines(element,opening);if(offset){const notice=document.createElement("span");notice.className="mapped-line tosafot-notice";notice.contentEditable="false";notice.setAttribute("aria-label","Fixed decorative Tosafos notice");notice.textContent=profile.notice.text;element.prepend(notice);fitMappedLineWidths(element);}element.contentEditable=state.mode==="edit"?"true":"false";element.spellcheck=false;}
  setMappedLines(regions.gemara[0],lines.gemara);
  setMappedLines(regions.inner[0],lines.inner.slice(openingLines,openingLines+narrowLines));
  const tosOpening=openingLines-(profile.notice?.stream==="tosafot"?noticeLines:0);
  setMappedLines(regions.tosafot[0],lines.tosafot.slice(tosOpening,tosOpening+narrowLines));
  const pairStart=openingLines+narrowLines;
  setMappedLines(regions.inner[1],lines.inner.slice(pairStart));
  const tosPairStart=tosOpening+narrowLines;
  setMappedLines(regions.tosafot[1],lines.tosafot.slice(tosPairStart,tosPairStart+pairLines));
  setMappedLines(regions[survivor][2],lines[survivor].slice(tosPairStart+pairLines));
...
function evaluate(pattern,scale,original,commit=false){setScale(scale);const remaining=flowOpening(original,commit),regions=buildGeometry(pattern),results={};for(const s of STREAMS)results[s]=fillStream(remaining[s],regions[s],commit);const sourceOverflow=STREAMS.reduce((n,s)=>n+results[s].rest.length,0),count=STREAMS.reduce((n,s)=>n+original[s].length,0)||1,blank=STREAMS.reduce((n,s)=>n+results[s].blankArea,0),area=STREAMS.reduce((n,s)=>n+results[s].totalArea,0)||1,blankRatio=blank/area,occupancies=STREAMS.flatMap(s=>results[s].occupancy),minOccupancy=Math.min(...occupancies),transitionOccupancies=[];for(const stream of STREAMS){const streamRegions=regions[stream],streamOccupancy=results[stream].occupancy;streamOccupancy.forEach((ratio,index)=>{const bandIndex=Number(streamRegions[index].dataset.band),continues=index<streamOccupancy.length-1,releasesEarly=index===streamOccupancy.length-1&&bandIndex<pattern.bands.length-1;if(continues||releasesEarly)transitionOccupancies.push(ratio);});}const transitionGap=transitionOccupancies.length?Math.max(...transitionOccupancies.map(r=>1-r)):0,gapPenalty=transitionOccupancies.reduce((n,r)=>n+Math.pow(Math.max(0,.97-r),2),0),terminalDeadSpace=Math.pow(Math.max(0,.9-minOccupancy),2),s=scaleParts(scale),scalePenalty=Math.abs(s.gemara-1)+Math.abs(s.commentary-1),score=sourceOverflow/count*32+blankRatio*10+terminalDeadSpace*110+gapPenalty*85+Math.pow(transitionGap,2)*140+(pattern.bands.length-1)*.004+scalePenalty*.035;let overflow=sourceOverflow;if(commit&&sourceOverflow){overflow=0;for(const stream of STREAMS){const rest=results[stream].rest,region=regions[stream].at(-1);if(rest.length&&!fitAllRemainingText(region,renderedTokens(rest,region)))overflow+=rest.length;}}return{score,overflow,sourceOverflow,blankRatio,minOccupancy,transitionGap,scale,pattern};}
function refinePatterns(pattern){if(pattern.bands.length<2)return[pattern];const refined=[];for(let firstDelta=-5;firstDelta<=5;firstDelta++){if(pattern.bands.length===2){const first=pattern.bands[0].height+firstDelta;if(first>10&&first<90)refined.push({...pattern,bands:[{...pattern.bands[0],height:first},{...pattern.bands[1],height:100-first}]});continue;}for(let secondDelta=-5;secondDelta<=5;secondDelta++){const first=pattern.bands[0].height+firstDelta,second=pattern.bands[1].height+secondDelta,third=100-first-second;if(first>10&&second>8&&third>5)refined.push({...pattern,bands:[{...pattern.bands[0],height:first},{...pattern.bands[1],height:second},{...pattern.bands[2],height:third}]});}}return refined;}
function candidatePasses(result){return result.overflow===0&&result.transitionGap<=.06&&result.blankRatio<.09&&result.minOccupancy>.72;}
function nextPaint(){return new Promise(resolve=>requestAnimationFrame(resolve));}
function setComposing(active){$("dafPage").classList.toggle("composing",active);$("dafPage").setAttribute("aria-busy",String(active));$("loadDaf").disabled=active;$("reflowEdits").disabled=active;$("typographyPreset").disabled=active;$("nekudosToggle").disabled=active;}
...
function validateMappedGeometry(){
  ...
  if(!gemara||Math.abs(parseFloat(getComputedStyle(gemara).paddingTop)-gutter)>.6)failures.push("25px top wall");
  if(bridge||!gemara||Math.abs(parseFloat(getComputedStyle(gemara).paddingBottom)-gutter)>.6)failures.push("25px bottom wall");
  const count=(stream,band)=>[...$("bodyGeometry").querySelectorAll(`[data-stream="${stream}"][data-band="${band}"] .mapped-line`)].length;
  if(count("gemara",0)!==profile.maps.gemara.lineEndTokens.length)failures.push("Gemara mapped-line placement");
  if(count("inner",0)!==profile.layout.narrowLines||count("tosafot",0)!==profile.layout.narrowLines)failures.push("narrow commentary placement");
  if(count("inner",1)!==profile.layout.pairLines-1||count("tosafot",1)!==profile.layout.pairLines)failures.push("paired commentary placement");
  if(count(profile.layout.survivor,2)!==profile.layout.fullLines)failures.push("full-width commentary placement");
  const mappedSource=streamTokens(),lineText=line=>line===null?"":line.map(token=>token.text).join(" ").replace(/\s+/g," ").trim();
  for(const stream of STREAMS){const expected=splitMappedLines(mappedSource[stream]).map(lineText),top=[...document.querySelectorAll(".top-commentary")].find(el=>el.dataset.stream===stream),containers=[...(top?[top]:[]),...$("bodyGeometry").querySelectorAll(`[data-stream="${stream}"]`)],actual=containers.flatMap(region=>[...region.querySelectorAll(":scope > .mapped-line:not(.tosafot-notice)")].map(line=>line.textContent.replace(/\s+/g," ").trim()));if(expected.length!==actual.length||expected.some((text,index)=>text!==actual[index]))failures.push(`${stream} mapped token preservation`);}
  const openingStrong=[...gemara?.querySelectorAll(":scope > .mapped-line:first-child strong")||[]],unpointed=text=>text.replace(/[\u0591-\u05C7]/g,"").replace(/\s+/g," ").trim();if(openingStrong.length!==1||!/^ערבי? פסחים$/u.test(unpointed(openingStrong[0].textContent)))failures.push("Gemara opening emphasis");
...
async function compose(){
  ...
  const w=weightsFor(tokens),patterns=candidates(w),globalScales=profile?[1]:[1.02,.99,.96,.93,.90,.87,.84,.81],total=patterns.length*globalScales.length;
  $("bodyGeometry").style.visibility="hidden";
  let best=null,bestPassing=null,done=0;const familyBest=new Map;
  for(const scale of globalScales)for(const pattern of patterns){
    const r=evaluate(pattern,scale,tokens),family=pattern.name;
  ...
  for(const r of[bestPassing||best,...mandatory,...winners])if(r&&!shortlist.some(x=>x.pattern.name===r.pattern.name)&&shortlist.length<4)shortlist.push(r);
  const fineJobs=shortlist.flatMap(coarse=>refinePatterns(coarse.pattern).flatMap(pattern=>[coarse.scale-.006,coarse.scale,coarse.scale+.006].map(scale=>({pattern,scale}))));
  done=0;for(const job of fineJobs){
    const r=evaluate(job.pattern,job.scale,tokens);if(r.score<best.score)best=r;if(candidatePasses(r)&&(!bestPassing||r.score<bestPassing.score))bestPassing=r;
  ...
  if(!bestPassing&&!profile){
    const recoveryPatterns=patterns.filter(p=>p.cascade),recoveryScales=[
      {gemara:1.10,commentary:.92},{gemara:1.08,commentary:.88},{gemara:1.06,commentary:.84},
      {gemara:1.04,commentary:.90},{gemara:1.02,commentary:.86},{gemara:1,commentary:.82}
    ],recoveryTotal=recoveryPatterns.length*recoveryScales.length;
    let recoveryBest=null;done=0;
    for(const scale of recoveryScales)for(const pattern of recoveryPatterns){
      const r=evaluate(pattern,scale,tokens);if(!recoveryBest||r.score<recoveryBest.score)recoveryBest=r;if(candidatePasses(r)&&(!bestPassing||r.score<bestPassing.score))bestPassing=r;
  ...
  const selected=bestPassing||best,final=evaluate(selected.pattern,selected.scale,tokens,true);
  $("bodyGeometry").style.visibility=originalBodyVisibility;final.failures=validateComposition(final);state.composition=final;state.dirty=false;
...
async function fetchText(ref,{commentary=false}={}){const url=`https://www.sefaria.org/api/texts/${encodeURIComponent(ref)}?context=0&commentary=0&pad=0&stripItags=0&alts=1`,res=await fetch(url);if(!res.ok)throw new Error(`Sefaria returned ${res.status}`);const data=await res.json();return{html:flattenSefaria(data.he,{commentary}),heRef:data.heRef||ref,heTitle:data.heTitle||""};}
function headerFor(ref,g){if(/^Pesachim\s+99b$/i.test(ref))return"ערבי פסחים פרק עשירי פסחים";return g.heTitle||g.heRef.replace(/[\d.:]+/g,"").trim()||ref;}
...
async function loadDaf({automatic=false}={}){const ref=$("dafRef").value.trim();if(!/\d+[ab]\s*$/i.test(ref)){status("Use a Talmud reference ending in a or b, such as Pesachim 99b.",true);return;}$("loadDaf").disabled=true;status("Step 1 of 2 — loading complete Gemara, Rashi, Tosafos, and available Rashbam from Sefaria…");try{const tractate=ref.replace(/\s+\d+[ab]\s*$/i,""),location=ref.match(/\d+[ab]\s*$/i)[0].trim(),commentary={commentary:true},requests=[fetchText(ref),fetchText(`Rashi on ${tractate} ${location}`,commentary),fetchText(`Tosafot on ${tractate} ${location}`,commentary),fetchText(`Rashbam on ${tractate} ${location}`,commentary).catch(()=>({html:""}))],[g,r,t,b]=await Promise.all(requests);if(!g.html)throw new Error("No Hebrew Gemara text was returned");const gemaraHtml=normalizeOpeningGemara(g.html,ref);Object.assign(state,{ref,header:headerFor(ref,g),gemaraHtml,rashiHtml:r.html,tosafotHtml:t.html,rashbamHtml:b.html,isSample:false,selectedWordId:null,editSelectedWordIds:[],wordFontScales:{},whitedWordIds:{},notes:{},annotations:[],annotationUndo:[],annotationRedo:[],selection:null});clearExcerpt();syncAnnotationCanvas();status(`Step 2 of 2 — text loaded${b.html?" with Rashbam":""}; starting the composition test…`);await nextPaint();await compose();}catch(e){setComposing(false);status(`${automatic?"Automatic import failed":"Could not load this daf"}: ${e.message}. The demonstration text remains available.`,true);}finally{$("loadDaf").disabled=false;}}
...
function editableRegionHtml(region){const mapped=[...region.children].filter(child=>child.classList.contains("mapped-line"));return mapped.length?mapped.filter(line=>!line.classList.contains("mapped-blank-line")&&!line.classList.contains("tosafot-notice")).map(line=>line.innerHTML).join(" "):region.innerHTML;}
...
function phraseAssignments(sourceWords,profile){
  const assignments=Array(sourceWords.length).fill(null);let cursor=0;
  for(let phraseIndex=0;phraseIndex<profile.length;phraseIndex++)for(const target of profile[phraseIndex].split(/\s+/u).filter(Boolean)){
    ...
    if(cursor>=sourceWords.length||!phraseWordMatches(sourceWords[cursor],normalizedTarget))return null;
    assignments[cursor]=phraseIndex;cursor++;
  }
  while(cursor<sourceWords.length&&isPhraseIgnoredValue(sourceWords[cursor]))cursor++;
  return cursor===sourceWords.length?assignments:null;
}
...
function setNavigationUnit(unit){
  ...
  updateNavigationUnitControls();applySelectionAppearance();updateNoteWindow();updateReadingControls();
}
function activateWordNavigation(){if(readingState.active)readingState.recognition?.abort();for(const old of $("dafPage").querySelectorAll(".word-token"))old.replaceWith(old.textContent);for(const stream of STREAMS){let index=0;for(const region of streamRegions(stream))index=wrapRegionWords(region,stream,index);}phraseNavigationAvailable=assignPhraseNavigation();updateNavigationUnitControls();applySelectionAppearance();resetReadingSession({keepSelection:true});setInteractionMode(state.mode,{focus:false});}
...
async function openProjectFile(file){const data=JSON.parse(await file.text());if(data?.format!=="vilna-daf-studio-project"||data.version!==1||!data.daf?.ref)throw new Error("This is not a supported Vilna Daf Studio project");Object.assign(state,{ref:String(data.daf.ref),header:String(data.daf.header||""),gemaraHtml:cleanHtml(data.daf.gemaraHtml),rashiHtml:cleanHtml(data.daf.rashiHtml),rashbamHtml:cleanHtml(data.daf.rashbamHtml),tosafotHtml:cleanHtml(data.daf.tosafotHtml),isSample:Boolean(data.daf.isSample),typography:TYPOGRAPHY_PRESETS[data.view?.typography]?data.view.typography:"archival-open",mode:data.view?.mode==="edit"?"edit":"navigate",navigationUnit:data.view?.navigationUnit==="phrase"?"phrase":"word",selectedWordId:data.view?.selectedWordId||null,editSelectedWordIds:[],wordFontScales:data.view?.wordFontScales&&typeof data.view.wordFontScales==="object"?data.view.wordFontScales:{},whitedWordIds:data.view?.whitedWordIds&&typeof data.view.whitedWordIds==="object"?data.view.whitedWordIds:{},notesEnabled:Boolean(data.view?.notesEnabled),nekudosEnabled:data.view?.nekudosEnabled!==false,lineNumbersEnabled:Boolean(data.view?.lineNumbersEnabled),pageZoom:normalizedPageZoom(data.view?.pageZoom),noteWindowPosition:data.view?.noteWindowPosition||null,selection:data.view?.selection||null,notes:data.notes&&typeof data.notes==="object"?data.notes:{},annotations:Array.isArray(data.annotations)?data.annotations:[],annotationTool:["pen","highlighter","eraser"].includes(data.annotationSettings?.tool)?data.annotationSettings.tool:"pen",annotationColor:data.annotationSettings?.color||"#b32424",strokeWidth:Number(data.annotationSettings?.width)||3,annotationUndo:[],annotationRedo:[]});$("dafRef").value=state.ref;$("annotationColor").value=state.annotationColor;$("strokeWidth").value=state.strokeWidth;$("strokeWidthValue").textContent=state.strokeWidth;updateDisplayToggles();selectAnnotationTool(state.annotationTool);setAnnotating(false);await applyTypography(state.typography,{recompose:false});await compose();toggleNotes(state.notesEnabled);if(state.selection){const b=$("selectionBox");b.hidden=false;Object.assign(b.style,{left:`${state.selection.x}px`,top:`${state.selection.y}px`,width:`${state.selection.width}px`,height:`${state.selection.height}px`});["copyExcerpt","downloadExcerpt","clearExcerpt"].forEach(id=>$(id).disabled=false);}$("projectStatus").textContent=`Opened ${file.name}.`;}
function afterCompose(){activateWordNavigation();updateDisplayToggles();requestAnimationFrame(()=>{applyPageZoom();syncAnnotationCanvas();renderGemaraLineNumbers();});updateNoteWindow();}
...
function svgToPng(svg){return new Promise((resolve,reject)=>{const img=new Image(),url=URL.createObjectURL(new Blob([svg],{type:"image/svg+xml"}));img.onload=()=>{const scale=3,c=document.createElement("canvas");c.width=Math.max(1,Math.round(state.selection.width*scale));c.height=Math.max(1,Math.round(state.selection.height*scale));const ctx=c.getContext("2d");ctx.fillStyle="#fffef9";ctx.fillRect(0,0,c.width,c.height);ctx.scale(scale,scale);ctx.drawImage(img,0,0);URL.revokeObjectURL(url);c.toBlob(b=>b?resolve(b):reject(new Error("PNG conversion failed")),"image/png");};img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error("SVG rendering failed"));};img.src=url;});}
async function copyExcerpt(){const svg=excerptForeignObjectSvg();if(!svg)return;try{const png=await svgToPng(svg);await navigator.clipboard.write([new ClipboardItem({"image/png":png})]);status("Caption-free daf excerpt copied at high resolution.");}catch{await downloadExcerpt();status("Clipboard image paste is unavailable, so the Canva SVG was downloaded.");}}
...
function clearExcerpt(){state.selection=null;$("selectionBox").hidden=true;["copyExcerpt","downloadExcerpt","clearExcerpt"].forEach(id=>$(id).disabled=true);}

$("loadDaf").addEventListener("click",()=>loadDaf());$("dafRef").addEventListener("keydown",e=>{if(e.key==="Enter")loadDaf();});$("reflowEdits").addEventListener("click",reflowPageEdits);$("excerptMode").addEventListener("click",beginExcerpt);$("copyExcerpt").addEventListener("click",copyExcerpt);$("downloadExcerpt").addEventListener("click",downloadExcerpt);$("clearExcerpt").addEventListener("click",clearExcerpt);$("typographyPreset").addEventListener("change",e=>applyTypography(e.target.value));
