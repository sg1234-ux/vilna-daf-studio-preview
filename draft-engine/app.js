...
function removeGemaraDashes(tokens){return tokens.flatMap(token=>{if(token.break)return[token];const parts=String(token.text||"").split(/[\u2010-\u2015\u2212-]/u).filter(Boolean);return parts.map(text=>({...token,text}));});}
function streamTokens(){const streams={gemara:tokenize(displayHtml(withoutBreaks(state.gemaraHtml))),inner:tokenize(displayHtml(innerHtml())),tosafot:tokenize(displayHtml(withoutBreaks(state.tosafotHtml)))},profile=referenceProfile();if(state.agentSettings.stripGemaraDashes&&!profile)streams.gemara=removeGemaraDashes(streams.gemara);if(profile)for(const stream of STREAMS)streams[stream]=applyExactLineMap(normalizeMappedSourceTokens(streams[stream],stream),profile.maps?.[stream],stream);return streams;}
