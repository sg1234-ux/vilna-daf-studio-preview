...
async function compose(){
  ...
  const w=weightsFor(tokens),patterns=candidates(w),globalScales=[1.02,1,.98,.96,.94,.92,.90],openingCandidates=state.agentSettings.openingLines?[state.agentSettings.openingLines]:[4,5,3,6],total=patterns.length*globalScales.length*openingCandidates.length;
  ...
  if(!bestPassing&&!profile){
    const recoveryPatterns=patterns.filter(p=>p.cascade),recoveryScales=[
      {gemara:1.08,commentary:.94},{gemara:1.06,commentary:.92},{gemara:1.04,commentary:.90}
